...
const ProfilePage = () => {
  ...
  // Hours State
  const [hoursRequired, setHoursRequired] = useState('600');
  const [isEditingHours, setIsEditingHours] = useState(false);
  const [tempHours, setTempHours] = useState('');

  const handleEditClick = () => {
    setTempHours(hoursRequired);
    setIsEditingHours(true);
  };

  const handleSaveHours = () => {
    setHoursRequired(tempHours);
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 text-blue-500">
                ...
                <span className="font-bold text-slate-100 tracking-tight">My Profile</span>
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase">Institution</p>
                  <p className="text-slate-200 font-medium">{userData.institution}</p>
                </div>
              </div>

              {/* Hours of OJT (Editable) */}
              <div className={`p-4 rounded-xl border flex items-center justify-between gap-4 transition-all duration-200 ${isEditingHours ? 'bg-slate-800 border-blue-500/50 ring-1 ring-blue-500/20' : 'bg-slate-950/50 border-slate-800'}`}>
                <div className="flex items-center gap-4 flex-1">
                  <div className="p-3 bg-emerald-500/10 rounded-lg text-emerald-400">
                    <Clock size={20} />
                  </div>
                  <div className="flex-1">
                    <p className="text-xs font-semibold text-slate-500 uppercase">Hours of OJT</p>
                    {isEditingHours ? (
                      <input 
                        type="number"
                        value={tempHours}
                      ...
                      <p className="text-slate-200 font-medium">{hoursRequired} Hours</p>
                    )}
                  </div>
                </div>
