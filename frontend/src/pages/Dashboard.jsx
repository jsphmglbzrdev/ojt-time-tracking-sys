...
const Dashboard = () => {
  ...
  const { logs, requiredHours, remainingHours, progress, totalHours, loading, fetchLogs, timeIn, timeOut, deleteLog } = useContext(TimeLogContext);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3 text-blue-500">
              ...
              <span className="text-lg font-bold text-slate-100 tracking-tight">InternTrack</span>
            </div>
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Student Intern</p>
                </div>
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-slate-900 rounded-xl shadow-xl border border-slate-800 py-1 animate-in fade-in zoom-in-95 duration-100 z-50">
                  <Link to="/dashboard/profile" className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800">
                    <User size={16} /> Profile
                  </Link>
									   <Link to="/dashboard/settings" className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800">
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <StatCard 
              title="Remaining" 
              value={`${calculatedRemainingHours.toFixed(1)}h`} 
              icon={<AlertCircle size={24} className="text-amber-400" />}
              trend={`${targetHours}h Goal`}
              color="amber"
            />
            <StatCard 
              title="Completion" 
              ...
              <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl">
                <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="bg-slate-800 text-slate-400 text-xs px-2 py-0.5 rounded-full border border-slate-700">{logs.length}</span>
                  </div>
                  <button className="text-sm text-blue-400 hover:text-blue-300 font-medium transition-colors">View History</button>
                </div>
                      ...
                      {loading ? (
                        ...
                        <tr>
                          <td colSpan="5" className="px-6 py-12 text-center text-slate-500">
                            ...
                            <p>No logs found. Start your shift to track time.</p>
                          </td>
                        </tr>
                      ) : (
                        logs.map((log) => {
                          ...
                          const timeInStr = timeInDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
                          const timeOutStr = timeOutDate ? timeOutDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false }) : '--:--';
                          const dateStr = timeInDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                          
                          return (
                            <tr key={log._id} className="hover:bg-slate-800/50 transition-colors group">
                              <td className="px-6 py-4 font-medium text-slate-200">{dateStr}</td>
                              <td className="px-6 py-4 text-slate-400">{timeInStr}</td>
                              <td className="px-6 py-4 text-slate-400">{timeOutStr}</td>
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl sticky top-24">
                ...
                <div className="grid grid-cols-7 gap-2">
                  {days.map((day, idx) => {
                    const hasLog = day && logs.some(l => l.date.includes(`Oct ${day}`));
                    return (
                      <div 
                        key={idx} 
                        className={`
                          aspect-square rounded-lg flex flex-col items-center justify-center relative transition-all text-sm
                          ${!day ? 'invisible' : 'cursor-pointer hover:bg-slate-800 border border-transparent hover:border-slate-700'}
                          ${day === today ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20 font-bold' : 'text-slate-300'}
                          ${hasLog && day !== today ? 'bg-slate-800/50' : ''}
                        `}
                      >
                        <span>{day}</span>
                        {hasLog && day !== today && (
        ...
        >
          <form onSubmit={handleTimeIn}>
            ...
            <p className="text-slate-400 mb-6 text-sm">
              Please confirm to clock in. The system will record your start time.
            </p>
            <div className="space-y-4 mb-6">
              <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                <p className="text-xs text-slate-500 uppercase font-semibold">Current Time</p>
                <p className="text-lg font-bold text-white mt-2">{getCurrentTime()}</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setModals({ ...modals, timeIn: false })} disabled={isProcessing} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl transition-colors disabled:opacity-50">Cancel</button>
              <button type="submit" disabled={isProcessing} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-emerald-500/20 disabled:opacity-50">
      ...
      {modals.timeOut && (
        ...
        >
          <form onSubmit={handleTimeOut}>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setModals({ ...modals, timeOut: false })} disabled={isProcessing} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl transition-colors disabled:opacity-50">Cancel</button>
              <button type="submit" disabled={isProcessing} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-red-500/20 disabled:opacity-50">
      ...
      {modals.delete && (
        ...
        >
          <div className="text-center mb-6">
            ...
            <p className="text-slate-300">Are you sure you want to delete this log entry? This action cannot be undone.</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setModals({ ...modals, delete: false })} disabled={isProcessing} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl transition-colors disabled:opacity-50">Cancel</button>
            <button onClick={confirmDelete} disabled={isProcessing} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-red-500/20 disabled:opacity-50">
      ...
      {modals.logout && (
        ...
        >
          <div className="text-center mb-6">
            ...
            <p className="text-slate-300">Are you sure you want to sign out of your account?</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setModals({ ...modals, logout: false })} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl transition-colors">Cancel</button>
            <button onClick={handleLogout} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-red-500/20">Sign Out</button>
          </div>
        </Modal>
      )}
